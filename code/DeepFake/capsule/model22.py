...
class FeatureExtractor(nn.Module):
    def __init__(self):
        ...
        self.capsules = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(2304, 64, kernel_size=3, stride=1, padding=1),
                nn.BatchNorm2d(64),
                nn.ReLU(),
...
class CapsuleNet(nn.Module):
    def __init__(self, num_class):
        ...
        self.routing_stats = RoutingLayer(num_input_capsules=NO_CAPS, num_output_capsules=num_class, data_in=8, data_out=4, num_iterations=2)
    ...
    def forward(self, input_v, random=False, dropout=0.0):

        z = self.effi.extract_features(input_v)
        z = self.fea_ext(z)
