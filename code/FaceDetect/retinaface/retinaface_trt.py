...
class Retinaface(object):
    ...
    def __init__(self, batch_size, load_path, **kwargs):
    ...
    def generate(self):
        ...
        self.inputs = torch.ones((self.batch_size, 3, self.input_shape[0], self.input_shape[1])).cuda()
        self.normalize = Normalize(mean=(104, 117, 123), std=(1, 1, 1), inplace=True).cuda()
    ...
    def predict_on_batch(self, image: np.ndarray or torch.Tensor):
        ...
        self.inputs[: image.shape[0]] = image.permute((0, 3, 1, 2))
        self.inputs = self.normalize(self.inputs)
        loc, conf, landms = self.net(self.inputs)
        loc = loc[: elen]
        conf = conf[: elen]
        landms = landms[: elen]
