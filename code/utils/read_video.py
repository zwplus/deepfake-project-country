...
class VideoReader:
    ...
    def read_frames(self, path, num_frames=-1, jitter=0, seed=None, second_num_frames=0):
        ...
        logger.info('video frame count: {}, fps: {}'.format(frame_count, fps))
        fps = max(fps, 12)
        if frame_count <= 0: return None

        # 最多提取256帧，不足256帧按照视频最大帧数处理
        ...
        logger.info('video read frames: {}'.format(num_frames))
